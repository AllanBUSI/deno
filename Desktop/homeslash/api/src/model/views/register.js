    ...
    register: async(req, res) => {
        ...
        if (error == true)
            ...
            if (await verif.emailExiste("user", data.email, res))
                status.sendReturn(res, 409, { error: true, message: "Un compte est déjà enregistré à cet email" });
                ...
                data.password = await new Promise(resolve => {
                    ...
                    sql.query(sql.insert("user"), toInsert, (error, results) => {
                        if (error) {
                            ...
                            if (res.status(201)) {
                                outil.email(data.email, "Ton compte HomeSlash", register.email(data.email, data.prenom));
